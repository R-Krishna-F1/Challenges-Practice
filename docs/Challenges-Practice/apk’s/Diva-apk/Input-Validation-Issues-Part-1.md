...
<empty-block/>
so we have to use sql injection for this challenge so i found the query is send to the data base of the table sqliuser
`Cursor cr = this.mDB.rawQuery("SELECT * FROM sqliuser WHERE user = '" + srchtxt.getText().toString() + "'", null);`
<empty-block/>
so here the query starts from 
...
I would remove this line which exposes db if the query is true
